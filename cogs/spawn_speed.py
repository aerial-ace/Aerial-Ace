...
class SpawnSpeedModule(commands.Cog):
    ...
    async def update_spawn_speeds(self):
        # return if locally cacheed spawnrate data is empty
    ...
    async def apply_spawn_counter(self, server_id: str = None):
        """Updates the spawn-speed channel name of each and every server or of the server provided"""

        d = cache_manager.manager.cached_spawnrate_data.items()

        if server_id is not None:
            d = [{server_id: cache_manager.manager.get_server(server_id)}]
        ...
        for x, y in d:
            ...
            try:
                ...
                continue  # Channel Not Found, or related error
            ...
            if isinstance(speed_display_channel, TextChannel) or isinstance(speed_display_channel, VoiceChannel) or isinstance(speed_display_channel, StageChannel):
                try:
                    ...
                    continue  # Unable to update the channel name, or related error
    ...
    async def sr_activate(self, context: commands.Context):
        data = cache_manager.manager.cached_spawnrate_data.get(str(context.guild.id), None)
