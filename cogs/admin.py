...
from managers import mongo_manager, cache_manager as cm
...
class AdminSystem(commands.Cog):
    ...
    async def show_data(self, ctx, collection: str, server_id: str):
        try:
            ...
            mongo_cursor = await cm.manager.get_server(int(server_id))
            end = time.perf_counter()
            elapsed = end - start
            print(f"Time taken is {elapsed:.6f} seconds")
    ...
    async def all_commands(self, ctx: commands.Context):
        ...
        embd.add_field(name="Page-1", value=strs[0], inline=True)

        embd.add_field(name="Page-2", value=strs[1], inline=True)

        embd.add_field(name="Page-3", value=strs[2], inline=True)
    ...
    async def set_value(self, ctx: commands.Context, server_id: str, key: str, value: str):
        ...
        updated_data = {".".join(key_tree[1:]): value}
    ...
    async def send_reminder(self, ctx: commands.Context, user_id: str, *, server_id: str):
        ...
        for i in server_ids:
            ...
            server_list.append({"name": gld.name, "id": gld.id})
