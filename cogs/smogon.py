...
class SmogonModule(commands.Cog):
    ...
    @commands.command(name="smogon", aliases=["analyze"], description="Returns complete usage analysis of a pokemon from smogon database")
    async def smogon_details(self, ctx:commands.Context, gen:int, tier:str, pokemon:str):
        ...
        if data is None:
            return await general_helper.get_info_embd("Error", "Some Unknown Error occurred while trying to fetch smogon data.", color=ERROR_COLOR)
